...
export class Game {
  ...
  private lock(){this.cells().forEach(pos=>this.board.set(key(pos),{pos,kind:this.kind}));const full=this.fullLayers();if(full.length)this.beginClear(full);else{this.spawn();this.emit()}}
